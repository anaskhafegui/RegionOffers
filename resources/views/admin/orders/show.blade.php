                ...
                <section class="invoice">
                    ...
                    <div class="row invoice-info">
                        <div class="col-sm-4 invoice-col">
                            طلب من
                            <address>
                                <i class="fa fa-angle-left" aria-hidden="true"></i>  @if(($order->client)){{$order->client->name}}@endif
                                <br>
                                <i class="fa fa-angle-left" aria-hidden="true"></i>   الهاتف : @if(($order->client)){{$order->client->phone}}@endif
                                <br>
                                <i class="fa fa-angle-left" aria-hidden="true"></i>  البريد الإلكترونى : @if(($order->client)){{$order->client->email}}@endif
                                <br>
                                <i class="fa fa-angle-left" aria-hidden="true"></i>  العنوان : {{$order->address}}
                            </address>
                        </div><!-- /.col -->
                        <div class="col-sm-4 invoice-col">المطعم 
                           <address>
                                <i class="fa fa-angle-left" aria-hidden="true"></i><strong>  @if($order->shop){{$order->shop->name}}@endif</strong>
                                <br>
                                <i class="fa fa-angle-left" aria-hidden="true"></i>  طريقة الشراء : @if($order->shop) {{$order->payment_method->name}}@endif
                                <br>
                                <i class="fa fa-angle-left" aria-hidden="true"></i>  الهاتف :  @if($order->shop) {{$order->shop->contact}}@endif
                            </address>
                        </div><!-- /.col -->
                        <div class="col-sm-4 invoice-col">
